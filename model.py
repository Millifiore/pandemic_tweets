...
covid_dataset = pd.concat([pos_label,neg_label]).reset_index(drop=True) #Concat positive and negative labels and reset the index
...
def pre_proc(series, tokenizer):  # Function for preprocessing data for encoding
    ...
    max_len = 0
    for i in tokenized.values:  # Finds the max len of the sentences
        if len(i) > max_len:
            max_len = len(i)
...
def encode(model, attention_mask, padded): #Function that runs the bert model and encodes the data
  ...
  features = last_hidden_states[0][:,0,:].numpy() #Creates features out of the encoding
